...
void writeFile(){
    ...
    for(int x = 0; x < numberOfShields; x++){
        int idx = x + numberOfWeapons + numberOfBows + 6;
        if(new_quantMod[idx] != quantMod[idx]){
            fseek(fp, FLAGSV_SHIELD[version] + (8 * x), SEEK_SET);
            fwrite(&new_quantMod[idx], sizeof(int), 1, fp);
        }
        if(new_modNames[idx] != modNames[idx]){
            fseek(fp, FLAGS_SHIELD[version] + (8 * x), SEEK_SET);
            fwrite(&new_modNames[idx], sizeof(int), 1, fp);
        }
    }
}
...
void getData(){
    ...
    for(int y = 0; y < 50; y++){
        ...
        for(int x = 0; x < 5; x++){
            ...
            if(len > 0 && tmpString[len - 1] == 2)
                tmpString[len - 1] = 0;

            strcat(itemName[y], tmpString);
        ...
        if(strcmp(itemName[y],"Armo")==0 || strlen(itemName[y])==0)
            break;
    ...
    for(int x = 0; x < numberOfShields; x++){
        int idx = x + numberOfWeapons + numberOfBows + 6;
        fseek(fp, FLAGS_SHIELD[version] + (8 * x), SEEK_SET);
        fread(&modNames[idx], sizeof(int), 1, fp);
        fseek(fp, FLAGSV_SHIELD[version] + (8 * x), SEEK_SET);
        fread(&quantMod[idx], sizeof(int), 1, fp);
...
int setFile(int intSlot){
    ...
    char file_name[75];
    snprintf(file_name, sizeof file_name, "save:/%d/game_data.sav", intSlot);
